...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h5 text-uppercase mb-0 text-gray-800">
        &nbsp;
        <a href="/dashboard/customer"><i class="fas fa-arrow-alt-circle-left mr-2"></i></a>
        <?= $title_bar; ?>
    </h1>
    <ol class="breadcrumb">
...
<div class="card shadow mb-4">
    <div class="card-body">
        <?= session()->get('pesan'); ?>
        <form action="/dashboard/insertCustomer" method="post">
            <?= csrf_field(); ?>
            <?php
            $rekeningModel = new \App\Models\KoderekeningModel();
            $pmg = $rekeningModel->where('rek_kode', '116')->first();
            $lastKode = $custModel->orderBy('cust_id', 'DESC')->first();
            ?>
            <input type="hidden" name="rekening" id="rekening" value="<?= $pmg['rek_id']; ?>">
            <input type="hidden" name="kode" id="kode" value="<?= (isset($lastKode) ? intval($lastKode['cust_kode']) + 1 : 1); ?>">
            <!-- <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select required name="rekening" id="rekening" class="form-control selectpicker <?= $validation->hasError('rekening') ? 'is-invalid' : ''; ?>" data-live-search="true">
                            ...
                            <?php
                            $rekeningModel = new \App\Models\KoderekeningModel();
                            $pmg = $rekeningModel->where('rek_kode', '112')->first();
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <input required type="text" name="kode" id="kode" class="form-control <?= $validation->hasError('kode') ? 'is-invalid' : ''; ?>" value="<?= old('kode') ? old('kode') : (isset($lastKode) ? intval($lastKode['cust_kode']) + 1 : 401); ?>">
                        <div class="invalid-feedback">

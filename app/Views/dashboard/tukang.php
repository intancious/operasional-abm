...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h5 text-uppercase mb-0 text-gray-800">
        &nbsp;<?= $title_bar; ?>
        <a href="#" class="addTukang" data-toggle="modal" data-target="#tukangModal"><i class="fas fa-plus-circle ml-2"></i></a>
        <a href="/dashboard/tukang"><i class="fas fa-sm fa-sync-alt fa-sm ml-2"></i></a>
    </h1>
    <ol class="breadcrumb">
                    ...
                    if ($tukang) {
                        foreach ($tukang as $row) : ?>
                            <tr class="text-uppercase">
                                <td><?= $i++; ?></td>
                                <td>
                                    <?php if (in_array("editTukang", $akses)) { ?>
                                        <a href="#" data-id="<?= $row['tk_id']; ?>" class="editTukang" data-toggle="modal" data-target="#tukangModal">
                                            <?= $row['tk_nama']; ?>
                                        </a>
                                    <?php } else {
                                        echo $row['tk_nama'];
                                ...
                                <td><?= $row['tk_alamat']; ?></td>
                                <td>
                                    <?php if (in_array("deleteTukang", $akses)) { ?>
                                        <form class="d-inline ml-1" action="/dashboard/deleteTukang" method="post">
                                            <?= csrf_field(); ?>
                                            <input type="hidden" name="_method" value="DELETE">
                                            <input type="hidden" name="id" value="<?= $row['tk_id']; ?>">
...
<div class="modal fade" id="tukangModal" tabindex="-1" role="dialog" aria-labelledby="tukangModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title text-uppercase" id="tukangModalLabel"></h5>

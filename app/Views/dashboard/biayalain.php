...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h5 text-uppercase mb-0 text-gray-800">
        &nbsp;<?= $title_bar; ?>
        <a href="#" class="addBiayalain" data-toggle="modal" data-target="#biayaLainModal"><i class="fas fa-plus-circle ml-2"></i></a>
        <a href="/dashboard/biayalain"><i class="fas fa-sm fa-sync-alt fa-sm ml-2"></i></a>
    </h1>
    <ol class="breadcrumb">
                        ...
                        foreach ($biayalain as $row) : ?>
                            <tr>
                                ...
                                <td class="text-uppercase">
                                    <?php if (in_array("editBiayalain", $akses)) { ?>
                                        <a href="#" data-id="<?= $row['bl_id']; ?>" class="editBiayalain" data-toggle="modal" data-target="#biayaLainModal">
                                            <?= $row['bl_nama']; ?>
                                        </a>
                                    <?php } else {
                                        echo $row['bl_nama'];
                                ...
                                <td>
                                    <?php if (in_array("deleteBiayalain", $akses) && $row['bl_id'] > 4) { ?>
                                        <form class="d-inline ml-1" action="/dashboard/deleteBiayalain" method="post">
                                            <?= csrf_field(); ?>
                                            <input type="hidden" name="_method" value="DELETE">
                                            <input type="hidden" name="id" value="<?= $row['bl_id']; ?>">
...
<div class="modal fade" id="biayaLainModal" tabindex="-1" role="dialog" aria-labelledby="biayaLainModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title text-uppercase" id="biayaLainModalLabel"></h5>

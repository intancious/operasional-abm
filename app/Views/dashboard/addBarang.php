...
<div class="card shadow mb-4">
    <div class="card-body">
        <?= session()->get('pesan'); ?>
        <form action="/dashboard/insertBarang" method="post">
            <?= csrf_field(); ?>
            <?php
            $rekeningModel = new \App\Models\KoderekeningModel();
            $pmg = $rekeningModel->where('rek_kode', '114.2')->first();
            $lastKode = $barangModel->orderBy('barang_id', 'DESC')->first();
            $kode = intval($lastKode['barang_kode']) == 2099 ? intval($lastKode['barang_kode']) + 200 : intval($lastKode['barang_kode']);
            ?>
            <input type="hidden" name="rekening" id="rekening" value="<?= $pmg['rek_id']; ?>">
            <input type="hidden" name="kode" id="kode" value="<?= isset($lastKode) ? $kode + 1 : 1; ?>">
            <!-- <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select required name="rekening" id="rekening" class="form-control selectpicker <?= $validation->hasError('rekening') ? 'is-invalid' : ''; ?>" data-live-search="true">
                            ...
                            foreach ($rekeningModel->where('rek_kode', '114.2')->orderBy('rek_kode', 'ASC')->findAll() as $row) { ?>
                                <option value="<?= $row['rek_id']; ?>" data-tokens="(<?= $row['rek_kode']; ?>) <?= $row['rek_nama']; ?>" <?= ($pmg['rek_id'] ? $pmg['rek_id'] : old('rekening')) == $row['rek_id'] ? 'selected' : ''; ?>>(<?= $row['rek_kode']; ?>) <?= strtoupper($row['rek_nama']); ?></option>
                            <?php } ?>
                        </select>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="kode" id="kode" class="form-control <?= $validation->hasError('kode') ? 'is-invalid' : ''; ?>" value="<?= old('kode') ? old('kode') : (intval($lastKode['barang_kode']) + 1); ?>">
                        <div class="invalid-feedback">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input required type="text" name="harga" id="harga" class="form-control <?= $validation->hasError('harga') ? 'is-invalid' : ''; ?>" value="<?= old('harga') ? old('harga') : 0; ?>">
                        <div class="invalid-feedback">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="jumlah" id="jumlah" class="form-control <?= $validation->hasError('jumlah') ? 'is-invalid' : ''; ?>" value="<?= old('jumlah') ? old('jumlah') : '0'; ?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');">
                        <div class="invalid-feedback">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="minstok" id="minstok" class="form-control <?= $validation->hasError('minstok') ? 'is-invalid' : ''; ?>" value="<?= old('minstok') ? old('minstok') : '0'; ?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');">
                        <div class="invalid-feedback">

...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h5 text-uppercase mb-0 text-gray-800">
        &nbsp;<?= $title_bar; ?>
        <a href="#" class="addKabar" data-toggle="modal" data-target="#kabarModal"><i class="fas fa-plus-circle ml-2"></i></a>
        <a href="/dashboard/kategoriBarang"><i class="fas fa-sm fa-sync-alt fa-sm ml-2"></i></a>
    </h1>
    <ol class="breadcrumb">
                        ...
                        foreach ($kabar as $row) : ?>
                            <tr>
                                ...
                                <td class="text-uppercase">
                                    <?php if (in_array("editKategoriBarang", $akses)) { ?>
                                        <a href="#" data-id="<?= $row['kabar_id']; ?>" class="editKabar" data-toggle="modal" data-target="#kabarModal">
                                            <?= $row['kabar_nama']; ?>
                                        </a>
                                    <?php } else {
                                        echo $row['kabar_nama'];
                                ...
                                <td>
                                    <?php if (in_array("deleteKategoriBarang", $akses)) { ?>
                                        <form class="d-inline ml-1" action="/dashboard/deleteKategoriBarang" method="post">
                                            <?= csrf_field(); ?>
                                            <input type="hidden" name="_method" value="DELETE">
                                            <input type="hidden" name="id" value="<?= $row['kabar_id']; ?>">
...
<div class="modal fade" id="kabarModal" tabindex="-1" role="dialog" aria-labelledby="kabarModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title text-uppercase" id="kabarModalLabel"></h5>
